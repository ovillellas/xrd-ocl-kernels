...
def eval_test(ngvec, npos, check_single=False):
    ...
    res_single = None if not check_single else xok.gvec_to_xy(*foo, single_precision=True)
    return res_orig, res_double, res_single


def run_test(ngvec, npos, chunk_size=(512, 4096), detail=0, check_single=False):
    orig, double, single = eval_test(ngvec, npos, check_single=check_single)
...
if __name__ == '__main__':
    args = parse_args();
    run_test(args.ngvec, args.npos, (args.chunk_ngvec, args.chunk_npos), args.detail_level, args.check_single);
